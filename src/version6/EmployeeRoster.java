...
public class EmployeeRoster {
    ...
    public void displayPayroll(int currentMonth) {
        for (Employee emp : empList) {
            ...
            if (emp.getBirthdayBonus(currentMonth) > 0) bonus = " (Bonus Applied)";

            System.out.printf("ID: %d | Name: %-25s | Payout: ₱%,.2f%s%n",
                    emp.getEmpID(), emp.getEmpName(), salary, bonus);
        }
    }
    ...
    public int countCE() {
        int c = 0;
        for (Employee emp : empList) {
            if (emp instanceof CommissionEmployee && !(emp instanceof BasePlusCommissionEmployee)) c++;
        }
        return c;
    }
